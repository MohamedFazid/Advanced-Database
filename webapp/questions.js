...
ORDER BY consistency_score DESC;
  ...
  // ============================================================
  // QUESTION 2:
  // Which teams significantly over- or under-perform relative to bookmaker expectations?
  // ============================================================
  {
    id: "q2-over-under-vs-bookmaker",
    title:
...
SELECT
  ...
  ROUND((SUM(tmp.actual_points) - SUM(tmp.expected_points)) / COUNT(*), 3) AS points_over_expected_per_game
...
ORDER BY win_comeback_pct DESC, non_loss_comeback_pct DESC;
  ...
  // ============================================================
  {
    id: "q4-shots-on-target-pressure",
    title:
...
ORDER BY season_name;
  ...
  // Part A: Team-level averages
  // Part B: Low/Medium/High aggression bands
  // ============================================================
  {
    id: "q7-aggression",
    title:
    ...
    parts: [
      {
        label: "Part A – Team-level aggression vs performance",
        description:
...
ORDER BY avg_aggression_index DESC, avg_points_per_match DESC;
      ...
      {
        label: "Part B – Outcome rates by aggression band",
        description:
...
ORDER BY aggression_tercile;
  ...
  // Part A: Referee tendencies (yellow/red split + consistency)
  // Part B: Outcomes by referee card-intensity group
  // ============================================================
  {
  id: "q8-referees-and-cards",
  title:
    "Q8: Do certain referees consistently award more cards and does this influence match outcomes?",
  description:
    "Compares referee card tendencies (Part A) and whether high-card referees show different outcome rates (Part B).",
  parts: [
    {
      label: "Part A – Referee card tendencies (yellow/red split)",
      description:
        "Referee-level summary: average yellows/reds per match, consistency across seasons (stddev), and min/max cards observed in a single match.",
      sql: `
WITH ref_match AS (
  SELECT
    m.referee_id,
...
ORDER BY avg_yellow_cards_per_match DESC;
    ...
    {
      label: "Part B – Outcome rates by referee card intensity (low/medium/high)",
      description:
        "Groups referees into LOW/MEDIUM/HIGH card intensity using weighted cards and compares home/draw/away outcome rates.",
      sql: `
WITH ref_avg AS (
  SELECT
    m.referee_id,
